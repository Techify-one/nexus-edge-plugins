...
3. Depois do merge, o workflow **Publish marketplace** publica automaticamente
   quando `marketplace.json` ou um `*/manifest.json` mudar. Também é possível
...
## Criar outro marketplace

Faça um fork deste repositório, remova as pastas dos plugins que não deseja
distribuir e edite `marketplace.json` com o nome, `owner/repository` e identidade
editorial do fork. Sempre que criar ou remover uma pasta de plugin, atualize o
lockfile com `pnpm install --lockfile-only` e inclua-o no commit. Adicione uma
chave Ed25519 própria como secret
`PLUGIN_SIGNING_PRIVATE_KEY` nas configurações do GitHub Actions. Depois, crie
cada plugin como uma pasta na raiz a partir de `.marketplace/templates/plugin`
e abra um pull request. Os diretórios de suporte e os workflows compartilhados
