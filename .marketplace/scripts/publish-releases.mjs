...
for (const id of readdirSync(".").sort()) {
  if (!/^[a-z][a-z0-9_]{1,31}$/u.test(id)) continue;
  const manifestPath = join(id, "manifest.json");
  const asset = join(id, "release", `${id}.plugin.zip`);
...
const sdkManifest = JSON.parse(
  readFileSync(".marketplace/packages/plugin-sdk/package.json", "utf8"),
);
