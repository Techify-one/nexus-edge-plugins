...
describe("Soletrando plugin", () => {
  it("ships an Installer-compatible AI manifest", () => {
    const manifest = JSON.parse(
      readFileSync("soletrando/manifest.json", "utf8"),
    ) as {
      id: string;
  ...
  it("unlocks a phase only after ten consecutive correct answers", () => {
    ...
    const repository = readFileSync("soletrando/src/repository.ts", "utf8");
  ...
  it("never persists audio and never renders the secret word in practice", () => {
    const migration = ["0001_init.sql", "0002_transcription_settings.sql"]
      .map((name) => readFileSync(`soletrando/migrations/d1/${name}`, "utf8"))
      .join("\n");
    const practice = readFileSync(
      "soletrando/frontend/PracticePage.tsx",
      "utf8",
    );
  ...
  it("bounds and safely logs Workers AI transcription latency", () => {
    const route = readFileSync("soletrando/src/index.ts", "utf8");
    const transcription = readFileSync(
      "soletrando/src/transcription.ts",
      "utf8",
    );
  ...
  it("keeps model selection in the administrator interface only", () => {
    const admin = readFileSync("soletrando/frontend/ChildrenPage.tsx", "utf8");
    const practice = readFileSync(
      "soletrando/frontend/PracticePage.tsx",
      "utf8",
    );
  ...
  it("renders child-friendly success and error feedback", () => {
    ...
    const messages = readFileSync("soletrando/frontend/i18n.ts", "utf8");
    ...
    expect(readFileSync("soletrando/src/index.ts", "utf8")).toContain(
